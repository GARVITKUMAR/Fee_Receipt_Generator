...
public class UNumber implements Comparable<UNumber> {
	 ...
	 */public UNumber(double v){
		 ...
		 // Using log base 10 to determine the characteristic
	...
	public double getDouble() {
		int numDigits = 17;							
		if ( d.length < 17 ) numDigits = d.length;	
		double result = 0;							
		double divisor = 1;							
		for (int ndx = 0; ndx < numDigits; ndx++) {	
			divisor *= 10.0;						
			result = result + d[ndx]/divisor;		
		}
		return result * Math.pow(10.0, dP);			
	}											
}
